...
void DrawLogoScreen(void)
{
    ...
    else if (state == 3)    // Draw "raylib" text-write animation + "powered by"
    {
        ...
        DrawText(TextSubtext("batata", 0, lettersCount), GetScreenWidth()/2 - 54, GetScreenHeight()/2 + 48, 50, Fade(BLACK, alpha));

        if (framesCounter > 20) DrawText("festa da", logoPositionX, logoPositionY - 27, 20, Fade(DARKGRAY, alpha));
    }
}
